...
class LinkedInDashboard {
    ...
    createAccountHealthElement(account) {
        ...
        div.innerHTML = `
            <div>
                <span class="font-medium text-gray-800">${account.name || account.accountId}</span>
                <span class="block text-sm text-gray-600">Health Score: ${account.healthScore}%</span>
            </div>
            <div class="text-right">
                <span class="block ${healthColor} font-medium">${account.status}</span>
                <span class="text-xs text-gray-500">${account.isOnline ? 'Online' : 'Offline'}</span>
            </div>
        `;
    ...
    updateMessageChart(chartData) {
        ...
        this.charts.messageChart.data.datasets[0].data = chartData.messagesSent || [];
