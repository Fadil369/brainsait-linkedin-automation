        ...
        try {
          const prompt = {
            messages: [
              ...
              {
                role: "user", 
                content: `Generate a ${messageType} message for: ${profileData.firstName}
          ...
          const generatedMessage = aiResponse.response || "Hello! I'd like to connect with you to discuss healthcare innovation opportunities.";

          // Store generated message in KV for analytics
          const messageKey = `message_${Date.now()}`;
          await env.BRAINSAIT_KV.put(messageKey, JSON.stringify({
          ...
          const healthcareTemplates = {
            en: {
              connection: `Hi ${profileData.firstName}! I'm reaching out to connect with fellow healthcare professionals. Your expertise in ${profileData.field || 'healthcare technology'} caught my attention. I'd love to discuss the latest innovations in MENA's healthcare sector and explore potential collaboration opportunities.`,
              follow_up: `Hello ${profileData.firstName}, Thank you for connecting! I'm passionate about advancing healthcare technology in the MENA region. Would you be interested in discussing emerging trends in ${profileData.field || 'digital health'} and how we can drive innovation together?`
            },
            ar: {
              connection: `مرحبا ${profileData.firstName}! أتواصل معك للاتصال مع زملاء المهنة في مجال الرعاية الصحية. خبرتك في ${profileData.field || 'تكنولوجيا الرعاية الصحية'} لفتت انتباهي. أود مناقشة أحدث الابتكارات في قطاع الرعاية الصحية في منطقة الشرق الأوسط وشمال أفريقيا.`,
              follow_up: `مرحبا ${profileData.firstName}، شكرا لك على التواصل! أنا شغوف بتطوير تكنولوجيا الرعاية الصحية في منطقة الشرق الأوسط وشمال أفريقيا. هل تود مناقشة الاتجاهات الناشئة في ${profileData.field || 'الصحة الرقمية'}؟`
            }
          };

          const fallbackMessage = healthcareTemplates[language]?.[messageType] || 
      ...
      if (method === "POST") {
        try {
          ...
          return new Response(
            JSON.stringify({
              success: true,
              message: "D1 database initialized with LinkedIn automation tables",
